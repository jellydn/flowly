/**
 * Provider-backed runners for the PR review pipeline. Prompts treat PR and
 * repository content as untrusted data and return unknown JSON for the domain
...
/** Create a provider-backed runner shared by all configured specialist roles. */
export function createSpecialistRunner(
  model: string,
  env: Record<string, string | undefined>,
): SpecialistRunner {
  const call = createModelCall(model, env);
  return async (role, context, signal) => {
    const reply = await call(formatSpecialistPrompt(role, context), signal);
...
/** Create the independently configured provider-backed advisor runner. */
export function createAdvisorRunner(
  ...
  return async (input, signal) =>
    parseJson((await call(formatAdvisorPrompt(input), signal)).content);
}
