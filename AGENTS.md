...
- `review/review-state.ts` and `review/review-state-store.ts` implement persistent review state via a hidden HTML-comment PR issue comment (`<!-- flue-review-state ... -->`). The store filters state comments to the expected bot account (`github-actions[bot]` by default, configurable via `REVIEW_BOT_LOGIN`) to prevent spoofing by untrusted PR participants. State persistence is best-effort (non-fatal on failure) to avoid duplicate reviews on retry.
