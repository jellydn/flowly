...
├── tests/             # Node test-runner tests (38 files + helpers)
...
- Contains: `types.ts`, `store.ts`, `orchestrator.ts`, `intake.ts`, `git.ts`, `implementation.ts`, `verification.ts`
...
- Contains: 38 `.test.ts` files + `helpers.ts` (fixture builders, tool invocation)
