...
- Factory functions named `create<Thing>` that return a configured object/tool: `createListFilesTool`, `createRepositoryReader`, `createKeywordJudge`, `createAgentFactoryImplementer`
...
- Result discriminators use tagged unions such as `{ ok: true; value } | { ok: false; issues }`; persisted factory and campaign states use explicit string unions
...
**Parameters:** Object-argument pattern for functions with many options (`runScenario(input)`, factory dependency objects) — named fields beat long positional lists
